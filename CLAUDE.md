...
- `src/app/page.tsx` keeps the bootstrap data and editable profile in React state; saving calls `saveProfile` (`PATCH /settings/profile`) and replaces the form with the response. Types are re-exported from `settings-api.ts`. Only the "Profil" tab (form) and "Sécurité" tab (sessions list from bootstrap, honouring `sources.sessions === "unavailable"`) are wired; every other tab renders an explicit "not yet available" state instead of simulating a save — keep it that way. Note the contract already declares `PATCH /settings/notifications|appearance|general`, so wiring those tabs is a UI-only change.
